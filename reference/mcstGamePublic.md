...
(Bogomolnaia and Moulin, 2010) associated with a minimum cost spanning
tree problem \\(N_0, C)\\, using a specified solution concept. The
public game, denoted as \\(N, v_C^u)\\ \\(\\or simply \\(N, v^u)\\\\)\\,
evaluates the cost of each coalition \\S \subseteq N\\ assuming that
there are no property rights on nodes. Therefore, agents in \\S\\ can
use the nodes of agents in \\N \setminus S\\ to connect to the source,
paying the corresponding arc costs.
...
- `v_u`: a numeric vector containing the characteristic function values
  \\v^u(S)\\ for all coalitions. If `method = "montecarlo"`, returns a
...
- `best_T`: a data frame mapping each coalition \\S\\ to the optimal
  superset \\T\\ that minimizes its cost. If `method = "montecarlo"`,
  returns a string indicating that lazy evaluation was used.
...
- `total`: the total cost of the MCST, \\v^u(N)\\.
...
The public game \\(N, v_C^u)\\ is a cooperative game with transferable
utility (TU game) where the cost for each coalition \\S \subseteq N\\ is
defined as the minimum private game cost among all its supersets \\T
\supseteq S\\, i.e.:

\$\$v_C^u(S) = \min\_{S \subseteq T} v_C^p(T),\$\$

where \\v_C^p(\cdot)\\ is the characteristic function of the private
game. This means computing \\v^u(S)\\ requires calculating the private
game costs for all possible supersets of \\S\\.
...
The core of \\v^u\\ coincides with the core of \\v^p\\ restricted to
nonnegative cost shares. Consequently, classical rules such as Bird,
Dutta-Kar, or the folk rule belong to the core of the public game for
any MCST problem.

However, unlike in the private game, the Shapley value of the public
game does not necessarily belong to the core of \\v^u\\ (Trudeau and
Vidal-Puga, 2019).
...
spanning tree problems. In: Algaba E, Fragnelli V, Sánchez-Soriano J
(eds) Chapters in Game Theory: The Shapley value, chapter 24. CRC Press,
...
for classical rules belonging to the core of \\v^u\\.

[`mcstGamePrivate`](https://luciasouto13.github.io/mcstprules/reference/mcstGamePrivate.md)
for the pessimistic alternative to the public game.

[`mcstGameIrred`](https://luciasouto13.github.io/mcstprules/reference/mcstGameIrred.md)
for a game that can be defined from either the private or the public
approach.
...
#> Solution: Shapley 
#>     1     2     3     4     5     6 
#>  4.25  7.75  3.83 10.08  6.75  6.33 
